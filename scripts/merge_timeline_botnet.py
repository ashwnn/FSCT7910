#!/usr/bin/env python3
"""
merge_timeline_botnet.py - Merge Timeline data with Botnet malware data.

This script simply concatenates the timeline events and botnet malware entries
into a single dataset for unified analysis. Each row is tagged with its source.

Output: A merged dataset with all timeline events and botnet entries.
"""
import pandas as pd
import os
...
OUTPUT_PATH = 'data/Timeline_Botnet_Merged.csv'
...
def main():
    """Main function to merge timeline and botnet data."""
    ...
    if botnet_df is None or botnet_df.empty:
        print(f"Error: No data found in {BOTNET_PATH}")
        return
    print(f"  Botnet: {len(botnet_df)} entries")
    
    # Add source column to identify data origin
    timeline_df['source'] = 'timeline'
    botnet_df['source'] = 'botnet'
    
    # Rename columns to create a unified schema
    # Timeline columns: event_id, date, event_type, title, description, ...
    # Botnet columns: incident_id, malware_family, category, first_seen, last_seen, ...
    ...
    botnet_df = botnet_df.rename(columns={
        'incident_id': 'id',
        'malware_family': 'name',
        'category': 'type',
    ...
    merged_df = pd.concat([timeline_df, botnet_df], ignore_index=True, sort=False)
    ...
    print(f"  Botnet entries: {len(botnet_df)}")
